...
function renderSubjects(){
  $('#matGrid').innerHTML=SUBJECTS.map(s=>{let h1=localStorage.getItem(`msj7-summary::${s.key}::1`),h2=localStorage.getItem(`msj7-summary::${s.key}::2`),fc=getFlashcards().filter(f=>f.subject===s.key&&!f.stale).length,theme=SUBJECT_THEMES[s.key]||{primary:'#C8336F',soft:'#FCE7EF',ink:'#7A2E4F'},done=[h1,h2].filter(Boolean).length;
    return `<article class="mat-card" id="mc-${s.key}" style="--mat-primary:${theme.primary};--mat-soft:${theme.soft};--mat-ink:${theme.ink}"><button type="button" class="mat-header" aria-expanded="false" aria-controls="mb-${s.key}" onclick="toggleSubjectCard(this)"><span class="mat-badge">${s.short}</span><span class="mat-info"><span class="mat-name">${s.name}</span><span class="mat-sub">${done}/2 resumos salvos · ${fc} flashcards</span></span><span class="mat-progress" aria-hidden="true"><i style="width:${done/2*100}%"></i></span><span class="chev" aria-hidden="true">›</span></button><div class="mat-body" id="mb-${s.key}"><div class="doc-row"><div class="stage">1ª etapa</div><div class="doc-main"><b>Anotações ${s.short}</b><small>Google Docs da matéria</small></div><span class="summary-status ${h1?'done':''}">${h1?'resumo salvo':'sem resumo'}</span><button class="btn" onclick="openSubject('${s.key}','1')">Abrir</button></div><div class="doc-row"><div class="stage">2ª etapa</div><div class="doc-main"><b>Anotações ${s.short}</b><small>Google Docs da matéria</small></div><span class="summary-status ${h2?'done':''}">${h2?'resumo salvo':'sem resumo'}</span><button class="btn" onclick="openSubject('${s.key}','2')">Abrir</button></div></div></article>`}).join('');
}
...
function addChecklistItem(eid){let input=$('#newChecklistItem'),text=input?.value.trim();if(!text)return;let arr=getEvents(),e=arr.find(x=>String(x.id)===String(eid));if(!e||e.readOnly)return;e.checks=e.checks||[];e.checks.push({text,done:false});setEvents(arr);if(input)input.value='';renderCalendar();renderHome();openEventDetail(eid)}
function removeChecklistItem(eid,idx){let arr=getEvents(),e=arr.find(x=>String(x.id)===String(eid));if(!e||e.readOnly)return;e.checks.splice(idx,1);setEvents(arr);renderCalendar();renderHome();openEventDetail(eid)}
