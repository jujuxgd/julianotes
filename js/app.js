...
function renderSubjects(){
  $('#matGrid').innerHTML=SUBJECTS.map(s=>{let h1=localStorage.getItem(`msj7-summary::${s.key}::1`),h2=localStorage.getItem(`msj7-summary::${s.key}::2`),fc=getFlashcards().filter(f=>f.subject===s.key&&!f.stale).length,theme=SUBJECT_THEMES[s.key]||{primary:'#C8336F',soft:'#FCE7EF',ink:'#7A2E4F'},done=[h1,h2].filter(Boolean).length;
    return `<article class="mat-card" role="button" tabindex="0" aria-label="Abrir ${escapeHtml(s.name)}" style="--mat-primary:${theme.primary};--mat-soft:${theme.soft};--mat-ink:${theme.ink}" onclick="openSubject('${s.key}','1')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openSubject('${s.key}','1')}"><div class="mat-header"><span class="mat-badge">${s.short}</span><span class="mat-info"><span class="mat-name">${s.name}</span><span class="mat-sub">${done}/2 resumos salvos · ${fc} flashcards</span></span><span class="chev" aria-hidden="true">›</span></div></article>`}).join('');
}
