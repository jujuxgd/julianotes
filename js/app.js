...
function renderSubjects(){
  $('#matGrid').innerHTML=SUBJECTS.map(s=>{let h1=localStorage.getItem(`msj7-summary::${s.key}::1`),h2=localStorage.getItem(`msj7-summary::${s.key}::2`),fc=getFlashcards().filter(f=>f.subject===s.key&&!f.stale).length,theme=SUBJECT_THEMES[s.key]||{primary:'#C8336F',soft:'#FCE7EF',ink:'#7A2E4F'};
    return `<article class="mat-card" style="--mat-primary:${theme.primary};--mat-soft:${theme.soft};--mat-ink:${theme.ink}"><div class="mat-header"><span class="mat-badge">${s.short}</span><span class="mat-info"><span class="mat-name">${s.name}</span><span class="mat-sub">${fc} flashcards</span></span></div><div class="mat-stages"><button type="button" class="mat-stage-btn ${h1?'has-summary':''}" onclick="openSubject('${s.key}','1')"><span class="mat-stage-label">1ª etapa</span><span class="mat-stage-status">${h1?'✓ resumo salvo':'sem resumo'}</span></button><button type="button" class="mat-stage-btn ${h2?'has-summary':''}" onclick="openSubject('${s.key}','2')"><span class="mat-stage-label">2ª etapa</span><span class="mat-stage-status">${h2?'✓ resumo salvo':'sem resumo'}</span></button></div></article>`}).join('');
}
